...
char mem[6*1024]; //za citanje i zapisuvanje
char pom[4*1024]; //dopolnitelno koristenje
int r,w;
int flag;
...
ssize_t my_read(struct file *filep, char *buff, size_t count, loff_t *offp) // ovoj red e ist
{
    ...
    int bajti_za_citanje;

        if( r >= count){
            bajti_za_citanje = count;
        }else{
            bajti_za_citanje = r;
        }

        int start = r-bajti_za_citanje;
        if(start < 0){
            return -EFAULT;
        }

        if((copy_to_user(buff, mem+start, bajti_za_citanje))){
            return -EFAULT;
        }

        memset(mem+start, 0 , bajti_za_citanje);
        r-=bajti_za_citanje;

        if(r==0){
            w=0;
        }
        return bajti_za_citanje;
...
ssize_t my_write(struct file *filep, const char *buff, size_t count, loff_t *offp) // ista za site
{
        ...
        if(count > 6*1024 - w){
            printk(KERN_INFO "Not enough space in memory\n");
            return 0;
        }
